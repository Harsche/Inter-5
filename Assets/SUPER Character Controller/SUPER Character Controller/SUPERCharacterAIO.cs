...
namespace SUPERCharacter{
                    ...
                    switch (cameraPerspective){
                        case PerspectiveModes._1stPerson:{
                            ...
                            targetAngles = Vector2.SmoothDamp(targetAngles,
                                targetAngles + (yawPitchInput *
                                                ((inputSensitivity / fovMod) * Mathf.Pow(cameraWeight * fovMod, 2))),
                                ref viewRotVelRef, (Mathf.Pow(cameraWeight * fovMod, 2)) * Time.deltaTime);
                        ...
                        case PerspectiveModes._3rdPerson:{
                            ...
                            headRot = Vector3.SmoothDamp(headRot,
                                headRot + ((Vector3) yawPitchInput * (inputSensitivity * Mathf.Pow(cameraWeight, 2))),
                                ref cameraPosVelRef, (Mathf.Pow(cameraWeight, 2)) * Time.deltaTime);
                            ...
                            if (enableMouseAndCameraDebugging){
                                Debug.Log(headRot);
                                Debug.DrawRay(cameraObstCheck.origin, cameraObstCheck.direction * maxCameraDistance,
                                    Color.red);
                                Debug.DrawRay(cameraObstCheck.origin, cameraObstCheck.direction * -currentCameraZ,
                                    Color.green);
                            }
                            ...
                            if (Physics.SphereCast(cameraObstCheck, 0.5f, out cameraObstResult, maxCameraDistInternal,
                                ...
                                currentCameraZ = Mathf.SmoothDamp(currentCameraZ, -(maxCameraDistInternal * 0.85f),
                                    ref cameraZRef, Time.deltaTime, 10);
                            }
                        }
                            break;
                    }
        ...
        void UpdateCameraPosition_3rdPerson(){
            ...
            if (!isIdle && !isSliding && currentGroundInfo.isGettingGroundInfo){
                ...
                transform.eulerAngles = Vector3.up * Mathf.MoveTowardsAngle(transform.eulerAngles.y,
                    (Mathf.Atan2(p_Rigidbody.velocity.x, p_Rigidbody.velocity.z) * Mathf.Rad2Deg), 10);
            }
        ...
        void GetGroundInfo(){
            ...
            if (currentGroundInfo.isInContactWithGround && enableFootstepSounds && shouldCalculateFootstepTriggers){
                if (currentGroundInfo.groundFromRay.collider is TerrainCollider){
                    ...
                    if (meshFilter == null && currentGroundInfo != null){
                        RaycastHit hit = currentGroundInfo.groundFromRay;
                        if (hit.transform.parent != null)
                            meshFilter = hit.transform.parent.GetComponent<MeshFilter>();
                    }

                    if (meshFilter != null) currentGroundInfo.currentMesh = meshFilter.sharedMesh;
                    if (currentGroundInfo.currentMesh && currentGroundInfo.currentMesh.isReadable){
                        ...
                        var groundRenderer = currentGroundInfo.groundFromRay.transform
                            .GetComponent<Renderer>();
                        if (groundRenderer != null){
                            currentGroundInfo.groundMaterial = groundRenderer.sharedMaterials[submesh];
                        }
                    }
                    else{
                        var meshRenderer = currentGroundInfo.groundFromRay.collider.GetComponent<MeshRenderer>();
                        if (meshRenderer == null)
                            currentGroundInfo.groundFromRay.collider.transform.parent.GetComponent<MeshRenderer>();
                        if (currentGroundInfo != null && meshRenderer != null)
                            currentGroundInfo.groundMaterial = meshRenderer.sharedMaterial;
                    }
                }
            }
            else{
                        ...
                        if ((toggleCrouch ? crouchInput_FrameOf : !crouchInput_Momentary) && !crouchOverride &&
                            ...
                            break;
                                   ...
                                   ((enableStaminaSystem && jumpingDepletesStamina)
                                       ...
                                       : true) && (enableSurvivalStats
                                       ? (!currentSurvivalStats.isDehydrated && !currentSurvivalStats.isStarving)
                                       : true)) || sprintOveride) && OverheadCheck()){
                            isCrouching = false;
                            isSprinting = true;
                            currentGroundSpeed = sprintingSpeed;
                            currentGroundMovementSpeed = GroundSpeedProfiles.Sprinting;
 ...
 (Key)EditorGUILayout.EnumPopup(new GUIContent("Perspective Switch Key", "The keyboard key used to switch perspective modes. Set to none if you do not wish to allow perspective switching"),t.perspectiveSwitchingKey);
                ...
                if (t.cameraPerspective == PerspectiveModes._1stPerson){
                    ...
                    t.rotateCharaterToCameraForward = EditorGUILayout.ToggleLeft(
                        ...
                        t.rotateCharaterToCameraForward);
                    t.standingEyeHeight = EditorGUILayout.Slider(
